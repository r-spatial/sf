...
#'  but merely set an attribute tag to an \code{sfc} object. The effect takes place in
#'  \link{st_as_binary} or, more precise, in the C++ function \code{CPL_write_wkb}, where
#'  simple feature geometries are being serialized to well-known-binary (WKB). This happens
#'  always when routines are called in GEOS library (geometrical operations or predicates),
#'  for writing geometries using \link{st_write}, \link{write_sf} or \link{st_write_db};
#'  also \link{aggregate} and \link{summarise} by default union geometries, which calls
#'  a GEOS library function. Routines in these libraries receive rounded coordinates, and
#'  possibly return results based on them. \link{st_as_binary} contains an example of a
#'  roundtrip of \code{sfc} geometries through WKB, in order to see the rounding happening
#'  to R data.
#'
