...
// Forward declares
inline Rcpp::List read_data(wkb_buf *wkb, bool EWKB, bool spatialite, int endian, bool addclass, int *type, uint32_t *srid);

inline void write_data(std::ostringstream& os, Rcpp::List sfc, int i, bool EWKB,
                int endian, const char *cls, const char *dim, double prec,
                int srid);


inline void wkb_read(wkb_buf *wkb, void *dst, size_t n) {
	if (n > wkb->size)
...
inline void read_spatialite_header(wkb_buf *wkb, uint32_t *srid, bool swap) {
	// we're at byte 3 now:
...
inline void read_gpkg_header(wkb_buf *wkb, uint32_t *srid, int endian) {
...
inline Rcpp::NumericMatrix read_multipoint(wkb_buf *wkb, int n_dims, bool swap,
...
inline Rcpp::List read_geometrycollection(wkb_buf *wkb, int n_dims, bool swap, bool EWKB = 0,
...
inline Rcpp::NumericVector read_numeric_vector(wkb_buf *wkb, int n, bool swap,
...
inline Rcpp::NumericMatrix read_numeric_matrix(wkb_buf *wkb, int n_dims, bool swap,
...
inline Rcpp::List read_matrix_list(wkb_buf *wkb, int n_dims, bool swap,
                            Rcpp::CharacterVector cls = "", bool *empty = NULL) {
...
inline Rcpp::List read_data(wkb_buf *wkb, bool EWKB = false, bool spatialite = false,
...
inline unsigned int make_type(const char *cls, const char *dim, bool EWKB = false, int *tp = NULL,
                       int srid = 0) {
	int type = 0;
...
inline void add_byte(std::ostringstream& os, char c) {
	os.write((char*) &c, sizeof(char));
}

inline void add_int(std::ostringstream& os, unsigned int i) {
	const char *cp = (char *)&i;
	os.write((char*) cp, sizeof(int));
}

inline double make_precise(double d, double precision) {
	if (precision == 0.0)
		return d;
...
inline void add_double(std::ostringstream& os, double d, double prec = 0.0) {
	d = make_precise(d, prec); // doubles are ALLWAYS coordinates
	const char *cp = (char *)&d;
	os.write((char*) cp, sizeof(double));
}

inline void write_vector(std::ostringstream& os, Rcpp::NumericVector vec, double prec) {
	for (int i = 0; i < vec.length(); i++)
		add_double(os, vec(i), prec);
}

inline void write_matrix(std::ostringstream& os, Rcpp::NumericMatrix mat, double prec) {
	add_int(os, mat.nrow());
...
inline void write_matrix_list(std::ostringstream& os, Rcpp::List lst, double prec) {
...
inline void write_multilinestring(std::ostringstream& os, Rcpp::List lst, bool EWKB = false,
...
inline void write_multipolygon(std::ostringstream& os, Rcpp::List lst, bool EWKB = false,
                        int endian = 0, double prec = 0.0) {
	Rcpp::CharacterVector cl_attr = lst.attr("class");
	const char *dim = cl_attr[0];
	add_int(os, lst.length());
...
inline void write_triangles(std::ostringstream& os, Rcpp::List lst, bool EWKB = false,
                     int endian = 0, double prec = 0.0) {
	Rcpp::CharacterVector cl_attr = lst.attr("class");
	const char *dim = cl_attr[0];
	add_int(os, lst.length());
...
inline void write_geometrycollection(std::ostringstream& os, Rcpp::List lst, bool EWKB = false,
                              int endian = 0, double prec = 0.0) {
	add_int(os, lst.length());
...
inline void write_multipoint(std::ostringstream& os, Rcpp::NumericMatrix mat,
...
inline void write_data(std::ostringstream& os, Rcpp::List sfc, int i = 0, bool EWKB = false,
...
inline Rcpp::CharacterVector get_dim_sfc(Rcpp::List sfc, int *dim = NULL) {
...
inline Rcpp::List hex_to_raw(Rcpp::CharacterVector cx) {
...
inline Rcpp::CharacterVector raw_to_hex(Rcpp::RawVector raw) {
...
inline Rcpp::List read_wkb(Rcpp::List wkb_list, bool EWKB = false, bool spatialite = false, int endian = 0) {
...
inline Rcpp::List write_wkb(Rcpp::List sfc, bool EWKB = false, int endian = 0,
