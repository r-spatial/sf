...
static SEXP sf_add_one_try(SEXP inSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::traits::input_parameter< Rcpp::NumericVector >::type in(inSEXP);
    rcpp_result_gen = Rcpp::wrap(add_one(in));
    return rcpp_result_gen;
END_RCPP_RETURN_ERROR
}
RcppExport SEXP sf_add_one(SEXP inSEXP) {
    SEXP rcpp_result_gen;
    {
        Rcpp::RNGScope rcpp_rngScope_gen;
        rcpp_result_gen = PROTECT(sf_add_one_try(inSEXP));
    }
...
static int sf_RcppExport_validate(const char* sig) { 
    static std::set<std::string> signatures;
    if (signatures.empty()) {
        signatures.insert("Rcpp::NumericVector(*add_one)(Rcpp::NumericVector)");
    }
    return signatures.find(sig) != signatures.end();
}

// registerCCallable (register entry points for exported C++ functions)
RcppExport SEXP sf_RcppExport_registerCCallable() { 
    R_RegisterCCallable("sf", "sf_add_one", (DL_FUNC)sf_add_one_try);
    R_RegisterCCallable("sf", "sf_RcppExport_validate", (DL_FUNC)sf_RcppExport_validate);
    return R_NilValue;
}
