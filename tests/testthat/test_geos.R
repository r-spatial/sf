...
test_that("st_g_relate works", {
  r1 = sf:::st_g_relate(st_sfc(list(st_point(c(0,0)))), st_sfc(list(st_linestring(rbind(c(0,0),c(1,1))))))
  library(sp)
...
test_that("st_g_isValid works", {
  expect_true( sf:::st_g_isValid(
  	st_sfc(list(st_polygon(list(cbind(c(0,1,1,0,0), c(0,0,1, 1,0))))))))
  expect_false(sf:::st_g_isValid(
  	st_sfc(list(st_polygon(list(cbind(c(0,1,1,.5,0),c(0,0,1,-1,0))))))))
})
