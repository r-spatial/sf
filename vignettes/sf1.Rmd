...
Each geometry type can also be a (typed) empty set, which contains zero coordinates (it is not clear how to represent an empty geometry for `POINT`). An empty geometry can be thought of as an analogue to missing (`NA`) attributes, NULL values or empty lists.
