...
(Note that the `sf::` prefix in `st_read` loads data from the `sf` package directly. It is a shortcut to access data packaged within `sf` for examples You can use `st_read(filename)` without the `sf::` prefix to read your data. Also note that shapefiles consist of more than one file, all with identical basename, which reside in the same directory.)
