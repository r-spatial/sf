...
data: it reads and writes vector and raster data from practically
...
with no tables), an error message is given. In case there are more
than one layers, the first layer is returned, but a message and a
warning are given:
...
The message points to the `st_list` command, which lists the driver
...
`st_list` has the option to count the number of features in case
...
layer; these are reported by `st_list`.

In case a layer contains geometries only but no attributes (fields),
...
driver manual of [gdal](FIXME:append_link). Multiple options can be
...
Conversion of simple features into corresponding `Spatial*` objects
is done using the `as` method, coercing to `Spatial`:
