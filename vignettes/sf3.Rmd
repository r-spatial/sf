...
There are cases where functions return objects with mixed geometry types, and downstream functions have difficulty handling them. For some of these cases, `st_cast` may help modifying their type.  For sets of geometry objects (`sfc`) and simple feature sets (`sf), `st_cast` can be used by specifying the target type, or without specifying it. 
...
We can for instance rotate the counties of North Carolina 90 degrees around their centroid, and shrink them with 60%:
```{r,fig=TRUE}
nc = st_read(system.file("shape/nc.shp", package="sf"))
cntrd = st_centroid(nc)
nc2 = (st_geometry(nc) - cntrd) * rot(pi/2) * .75 + cntrd
plot(nc2)
plot(cntrd, col = 'red', add = TRUE)
```
