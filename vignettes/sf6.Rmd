...
This works up to the moment that a `dplyr` method for an `sf` object
is needed: should one call `dplyr::select` (won't know it should search
in package `sf`) or `sf::select` (which doesn't exist)? Neither. One should
in this case simply load `sf`, either by
```{r,eval=FALSE}
library(sf)
```
