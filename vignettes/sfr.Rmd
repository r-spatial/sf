...
Only the first 7 geometry types are described; the remaining 10 are found rarely, but are part of the standard and software support is increasing for them. Each of the geometry types can also be a (typed) empty set, containing zero coordinates
(or for `POINT` containing missing values). Empty geometries can be thought of being similar to missing (`NA`) attributes.
...
plot(a, ylim = c(-1,1))
title("intersecing two polygons:")
...
(units; support, spatially extensive/intensive properties, and the consequences for meaningful geometrical ops)
