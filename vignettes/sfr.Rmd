...
R has has well-supported classes for storing spatial
...
[rgeos](http://cran.r-project.org/package=rgeos)), but lacked a
...
* discusses how they can be converted from sp objects
...
[wkb]: Well-known text and well-known binary are two encodings for simple feature geometries. Well-known text, e.g. seen in
```{r}
...
documentation is best consulted for this.  For instance, a PostGIS table in database `postgis` might
be read by
```{r eval=FALSE}
...
```{r}
st_intersects(nc[1:5,])
st_intersects(nc[1:5,], sparse = FALSE)
```
...
a = st_sfc(st_polygon(list(cbind(c(0,0,7.5,7.5,0),c(0,-1,-1,0,0)))))
b = st_sfc(st_polygon(list(cbind(c(0,1,2,3,4,5,6,7,7,0),c(1,0,.5,0,0,0.5,-0.5,-0.5,1,1)))))
plot(a)
...
(this will eventually be the topic of a new vignette; now for completeness)
