...
| `TRIANGLE` | A Triangle is a polygon with 3 distinct, non-collinear vertices and no
interior boundary |
...
which is `sfc` for all types, and `sft_TYPE` with `TYPE` indicating the type of each of the geometries. There are two "special" types: `GEOMETRYCOLLECTION`, indicating that each of the geometries may contain a mix of geometry types, 
```{r}
...
and `GEOMETRY`, indicating that the geometries in the geometry list-column are of mixed type:
```{r}
