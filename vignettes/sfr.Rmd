...
(e.g. `XYM`) class name. A figure illustrates the seven most
common types:
...
imprecise. For that, we use well-known binary (WKB) encoding
...
copies from JTS, and works like this:
...
For the precion model, see also
[here](http://tsusiatsoftware.net/jts/javadoc/com/vividsolutions/jts/geom/PrecisionModel.html),
...
```
The `dsn` and `layer` arguments to `st_read` and `st_write` denote a
data source name and a layer.  Their exact interpretation as well
as the options they support vary per driver, the [GDAL driver
documentation](http://www.gdal.org/ogr_formats.html) is best
consulted for this.  For instance, a PostGIS table in database
...
read and write directly from spatial databases, using WKB. The
advantage over `st_read` may be that instead of a complete table,
the result of a (spatial) query may be fetched, limiting the amount
of data that is read into R.
...
```
Benchmarks show that `st_read()` is often faster than `rgdal::readOGR()`:
...
This might give a table like this:
```
Unit: milliseconds
          expr      mean
 shp_read_sp() 4645.4947 
 shp_read_sf()  501.0872
 kmz_read_sp() 6256.2638
 kmz_read_sf() 2169.9361
```
...
web mercator (EPSG:3875) can be done by
