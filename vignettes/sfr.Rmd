...
The remaining geometries 10 are more rare, but are increasingly find implementations:
...
Note that `CIRCULASTRING`, `COMPOUNDCURVE` and `CURVEPOLYGON` are not described in the standard.
...
in a `data.frame` column. Since geometries are not single-valued,
they are put a list-column, a list of length equal to the number
of records in the `data.frame`, with each list element the simple
...
st_drop_zm(x)
```
which illustrates that we can represent 2-, 3- or 4-dimensional coordinates. All geometry objects inherit from `sfg` (simple feature geometry), but also have a type (e.g. `POINT`), and a dimension (e.g. `XYM`) class name. A figure illustrates the seven most common types:
...
in a `data.frame`) are shown here:

```{r echo=FALSE}
set.seed(1333)
par(mar = c(0.1,0.1,1.3,0.1), mfrow = c(2, 3))

plot(st_multipoint(matrix(rnorm(12),,2)), col = 'red')
box()
title("MULTIPOINT")

s1 = rbind(c(0,2), c(2,2), c(3,3), c(4,2), c(3,1), c(3,2))
plot(st_linestring(s1), col = 'red')
box()
title("LINESTRING")

s2 = rbind(c(0,1), c(1,1), c(3,0), c(4,0), c(4,1))
s3 = rbind(c(0,3), c(2,3))
plot(st_multilinestring(list(s1,s2,s3)), col = 'red')
box()
title("MULTILINESTRING")

x1 = rbind(c(0,0), c(1,0), c(3,2), c(2,4), c(1,4), c(0,0))
x2 = rbind(c(1,1), c(1,2), c(2,2), c(1,1))
plot(st_polygon(list(x1,x2)), border = 'red', col = 'grey', xlim = c(0,4))
box()
title("POLYGON")

x3 = rbind(c(3,0), c(4,0), c(4,1), c(3,1), c(3,0))
x3a = rbind(c(3.3,0.3), c(3.8,0.3), c(3.8,0.8), c(3.3,0.8), c(3.3,0.3))[5:1,]
x4 = rbind(c(3,3), c(4,2), c(4,3), c(3,3))
plot(st_multipolygon(list(list(x1,x2), list(x3,x3a), list(x4))), border = 'red', col = 'grey')
box()
title("MULTIPOLYGON")

gc1 = st_multipolygon(list(list(x1,x2), list(x3,x3a), list(x4)))
gc2 = st_point(c(2,0))
gc4 = st_multipoint(rbind(c(3,4), c(3.3, 4), c(3.1, 4.4), c(3.3, 4.2)))
gc3 = st_linestring(rbind(c(0,3),c(0,4),c(1,5),c(2,5)))
plot(st_geometrycollection(list(gc1,gc2,gc3,gc4)), border = 'grey', col = 'grey')
box()
